...
import { Activity, CheckCircle2, Server, Database, Shield, Bell, RefreshCw, Clock } from 'lucide-react';
import PageHeader from '../../common/PageHeader';

const SystemStatus = () => {
  const services = [
    { name: 'Core API Gateway', status: 'Operational', uptime: '99.98%', latency: '24ms', icon: Server, color: 'text-emerald-500' },
    { name: 'MongoDB Database Cluster', status: 'Operational', uptime: '99.99%', latency: '12ms', icon: Database, color: 'text-emerald-500' },
    { name: 'Authentication & JWT Service', status: 'Operational', uptime: '100.0%', latency: '18ms', icon: Shield, color: 'text-emerald-500' },
    { name: 'Notification & Alerts Engine', status: 'Operational', uptime: '99.95%', latency: '35ms', icon: Bell, color: 'text-emerald-500' },
  ];
    ...
    <div className='space-y-8 animate-fade-in pt-0 pb-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
      ...
      <div className='rounded-2xl border border-emerald-200 dark:border-emerald-900/60 bg-emerald-50/70 dark:bg-emerald-950/30 p-6 flex flex-col sm:flex-row items-center justify-between gap-4'>
        <div className='flex items-center gap-4'>
          ...
          <div>
            ...
            <p className='text-xs text-slate-600 dark:text-slate-300'>All academic platform microservices are running normally.</p>
          </div>
        </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
        {services.map((svc) => {
          const Icon = svc.icon;
          return (
            <div key={svc.name} className='rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 space-y-4 shadow-sm'>
              <div className='flex items-center justify-between'>
                <div className='flex items-center gap-3'>
                  ...
                  <div>
                    ...
                    <span className='text-[11px] text-slate-500 dark:text-slate-400'>Latency: {svc.latency}</span>
                  </div>
                </div>
