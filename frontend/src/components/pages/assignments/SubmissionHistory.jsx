  ...
  if (loading) {
    return (
      <div className="assignment-page flex flex-col items-center justify-center">
        <p className="assignment-subtitle italic">Accessing submission archives...</p>
      </div>
    );
  }
            ...
            <table className="assignment-table">
              ...
              <tbody>
                {submissions.map((item) => (
                  <tr key={item.id}>
                    <td>
                      <div className="font-bold text-gray-900 dark:text-white">{item.assignment}</div>
                    </td>
                    <td><div className="text-gray-600 dark:text-gray-400 font-medium">{item.course}</div></td>
                    <td><div className="text-gray-600 dark:text-gray-400">{item.submittedDate}</div></td>
                    <td>
                    ...
                    <td>
                      <span className={`assignment-badge ${
                        item.status === "Graded" ? "badge-submitted" : "badge-progress"
                      }`}>
                        {item.status}
                      </span>
                    </td>
                    <td><div className="text-gray-600 dark:text-gray-400 font-bold">{item.filesCount}</div></td>
