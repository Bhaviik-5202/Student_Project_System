      ...
      {loading ? (
        ...
        <div className='rounded-2xl border border-slate-200 bg-white dark:bg-slate-900 p-6 md:p-8 shadow-sm dark:border-slate-700 dark:bg-slate-800'>
          <form onSubmit={handleSubmit} className='space-y-6'>
            ...
            {/* Form Actions */}
            <div className='flex flex-col-reverse sm:flex-row items-center justify-end gap-3 border-t border-slate-100 pt-6 dark:border-slate-700'>
              <button
                type='button'
                onClick={() => navigate('/user-management')}
                className='w-full sm:w-auto rounded-xl border border-slate-200 bg-white dark:bg-slate-900 px-5 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors'
              >
                Cancel
              </button>
              <button
                type='submit'
                disabled={submitting}
                className='w-full sm:w-auto flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700 transition-colors shadow-md shadow-indigo-100 dark:shadow-none disabled:opacity-50'
              >
                {submitting ? (
                  <>
