      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <StatisticsCard
              title="System Health"
              value={stats.systemHealth ? `${stats.systemHealth}% Operational` : '100% Operational'}
              icon={Activity}
              color="emerald"
                ...
                {performanceData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={performanceData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      ...
                      />
                      <Line type="monotone" dataKey="projects" stroke="#3b82f6" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} name="Total Projects" />
                      <Line type="monotone" dataKey="completions" stroke="#10b981" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} name="Completed" />
                    </LineChart>
                  </ResponsiveContainer>
                ) : (
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white dark:bg-slate-900 p-6 shadow-xs dark:border-slate-800 ">
              ...
              <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
                <button onClick={() => window.location.href = '/user-management/new'} className="flex flex-col items-center justify-center gap-2 rounded-xl border border-slate-100 bg-slate-50 dark:bg-slate-800 p-4 transition-all hover:border-indigo-200 hover:bg-indigo-50 dark:border-slate-800 /40 dark:hover:border-indigo-900 dark:hover:bg-indigo-900/20">
                  ...
                  <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">Add User</span>
                </button>
                <button onClick={() => window.location.href = '/reports'} className="flex flex-col items-center justify-center gap-2 rounded-xl border border-slate-100 bg-slate-50 dark:bg-slate-800 p-4 transition-all hover:border-blue-200 hover:bg-blue-50 dark:border-slate-800 /40 dark:hover:border-blue-900 dark:hover:bg-blue-900/20">
                  ...
                  <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">View Reports</span>
                </button>
                <button onClick={() => window.location.href = '/system-settings'} className="flex flex-col items-center justify-center gap-2 rounded-xl border border-slate-100 bg-slate-50 dark:bg-slate-800 p-4 transition-all hover:border-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700  dark:border-slate-800 /40 dark:hover:border-slate-700 dark:hover:bg-slate-800">
                  ...
                  <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">System Config</span>
                </button>
                <button onClick={() => window.location.href = '/audit-log'} className="flex flex-col items-center justify-center gap-2 rounded-xl border border-slate-100 bg-slate-50 dark:bg-slate-800 p-4 transition-all hover:border-emerald-200 hover:bg-emerald-50 dark:border-slate-800 /40 dark:hover:border-emerald-900 dark:hover:bg-emerald-900/20">
                  ...
                  <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">Full Audit Log</span>
                </button>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white dark:bg-slate-900 p-6 shadow-xs dark:border-slate-800  flex flex-col">
              ...
              <div className="mt-4 flex-1 space-y-3">
                {todayMeetings.length > 0 ? todayMeetings.map((meeting) => (
                  <div key={meeting.id} className="flex items-start gap-4 rounded-xl border border-slate-100 bg-slate-50 dark:bg-slate-800 p-4 dark:border-slate-800 /40">
                    <div className="flex flex-col items-center justify-center rounded-lg bg-indigo-100 px-3 py-2 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-400">
                      <span className="text-xs font-bold uppercase">{new Date(meeting.date).toLocaleDateString('en-US', { month: 'short' })}</span>
                      <span className="text-lg font-black">{new Date(meeting.date).getDate()}</span>
                    </div>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                {auditLogs.length > 0 ? auditLogs.map((log) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-slate-900 dark:text-white text-xs">{log.action || log.activity}</p>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                          By <span className="font-medium text-slate-700 dark:text-slate-300">{log.user?.name || log.user?.email || 'System'}</span>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-slate-900 dark:text-white text-xs">{log.title}</p>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                          {log.description}
                        </p>
                      </div>
                    ...
                    <span className="text-[10px] font-medium text-slate-400">
                      {log.updatedAt ? new Date(log.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}
                    </span>
                  </div>
                )) : (
            ...
            <div className="rounded-2xl border border-slate-200 bg-white dark:bg-slate-900 p-6 shadow-xs dark:border-slate-800 ">
              <div className="flex items-center justify-between">
                ...
                />
                <button onClick={() => window.location.href = '/user-management'} className="flex items-center gap-1 text-xs font-semibold text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">
              ...
              <div className="mt-4 overflow-x-auto">
                <table className="w-full text-left text-xs text-slate-600 dark:text-slate-400">
                  <thead className="border-b border-slate-200 bg-slate-50 dark:bg-slate-800 uppercase text-slate-500 dark:text-slate-400 dark:border-slate-800 /50">
                    <tr>
                      <th className="py-3 px-4 font-semibold rounded-tl-xl">User</th>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {users.length > 0 ? users.slice(0, 5).map((u, idx) => (
                      <tr key={u.id || u._id || idx} className="hover:bg-slate-50 dark:hover:bg-slate-800 dark:bg-slate-800/50 /50 transition-colors">
                        ...
                        <td className="py-3 px-4">
                          <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${u.role === 'admin' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' :
                              u.role === 'faculty' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400' :
