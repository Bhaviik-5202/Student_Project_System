...
const StatCard = memo(({ icon: IconComponent, iconColorClass, bgColorClass, value, label, delay = 0 }) => {
  ...
  return (
    ...
    >
      <div className="flex items-center">
        <div className={`w-14 h-14 ${bgColorClass} rounded-xl flex items-center justify-center mr-4 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3`}>
          <IconComponent className={`w-7 h-7 ${iconColorClass}`} aria-hidden="true" />
        </div>
        <div>
      ...
      <div className={`h-1 mt-4 rounded-full ${bgColorClass} overflow-hidden`}>
        <div 
...
const QuickActionButton = memo(({ icon: IconComponent, iconColorClass, bgColorClass, label, onClick }) => (
  <button
    onClick={onClick}
    ...
    aria-label={label}
  >
    <div className={`absolute inset-0 ${bgColorClass || 'bg-blue-50 dark:bg-blue-900/20'} opacity-0 group-hover:opacity-100 transition-opacity duration-300`} />
    <div className="relative z-10">
      ...
      <div className="font-semibold text-slate-900 dark:text-white transition-colors">{label}</div>
    </div>
  </button>
));
...
const ActivityItem = memo(({ user, action, time, icon: IconComponent }) => (
  <div className="flex items-center justify-between p-4 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-all duration-200 hover:border-blue-200 dark:hover:border-blue-800 cursor-pointer group">
    <div className="flex items-center gap-3">
      ...
      <div>
        <div className="font-semibold text-slate-900 dark:text-white">{user}</div>
        <div className="text-sm text-slate-600 dark:text-slate-400">{action}</div>
      </div>
    </div>
    <time className="text-sm text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 px-3 py-1 rounded-full">{time}</time>
  </div>
));
  ...
  return (
    <div className="flex items-center justify-between p-4 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-all duration-200 group">
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="font-semibold text-slate-900 dark:text-white">{service}</div>
      ...
      <div className="text-right">
        <div className="text-lg font-bold text-slate-900 dark:text-white">{uptime}</div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="relative mb-8 overflow-hidden rounded-2xl bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 p-8 text-white shadow-xl">
          ...
          <div className="absolute inset-0 overflow-hidden">
            ...
            <div className="absolute -bottom-16 -left-16 w-48 h-48 bg-white/10 rounded-full blur-2xl animate-pulse" style={{ animationDelay: '1s' }} />
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="bg-white/20 backdrop-blur-sm rounded-xl px-4 py-2 text-center">
                <div className="text-2xl font-bold text-emerald-300">All OK</div>
        ...
        <section aria-label="Statistics Overview" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {STATS_CONFIG.map(({ key, icon, iconColorClass, bgColorClass, label, suffix }, index) => (
            <StatCard
              key={key}
              icon={icon}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          >
            ...
            <nav className="grid grid-cols-2 gap-4" aria-label="Admin navigation">
              {QUICK_ACTIONS_CONFIG.map(({ id, icon, iconColorClass, bgColorClass, label, path }) => (
                <QuickActionButton
                  key={id}
                  icon={icon}
                  iconColorClass={iconColorClass}
