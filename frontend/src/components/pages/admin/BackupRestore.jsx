...
export const BackupRestore = () => {
  ...
  const fetchBackups = useCallback(async () => {
    ...
    try {
      ...
      if (res.success || res.data) {
        setBackups(
          Array.isArray(res.data) ? res.data : res.data?.backups || []
        );
      }
    } catch (err) {
  ...
  const handleRestore = async (backup) => {
    ...
    if (
      !window.confirm(
        `RESTORE WARNING: Are you sure you want to restore the system state to "${backup.name || backupId}"?`
      )
    )
      return;
          ...
          <div className='w-full sm:w-64'>
            <Select
              ...
              options={[
                {
                  value: 'Full',
                  label: 'Full System Snapshot (Database + Files)',
                },
