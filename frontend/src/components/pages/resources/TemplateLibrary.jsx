  ...
  const handleDelete = async (tpl) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setTemplates((prev) => prev.filter((t) => (t._id || t.id) !== (tpl._id || tpl.id)));
      } else {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <StatisticsCard
          title="Slide Decks"
          value={templates.filter((t) => t.fileType === 'pptx' || t.title.toLowerCase().includes('presentation')).length}
          icon={Presentation}
          color="amber"
        ...
        <StatisticsCard
          title="SRS & Reports"
          value={templates.filter((t) => t.fileType === 'docx' || t.title.toLowerCase().includes('srs')).length}
          icon={FileCode}
          color="emerald"
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {filteredTemplates.map((tpl) => {
            const isPptx = tpl.fileType === 'pptx' || tpl.title.toLowerCase().includes('presentation');
            const isDocx = tpl.fileType === 'docx' || tpl.title.toLowerCase().includes('srs');
              ...
              >
                <div>
                  ...
                  <p className="mt-1 line-clamp-2 text-xs text-slate-500 dark:text-slate-400">
                    {tpl.description || 'Standardized template for student project requirements.'}
                  </p>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-xs">
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-xl dark:border-slate-800 dark:bg-slate-900">
            ...
            <form onSubmit={handleUpload} className="mt-4 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase">Template Title</label>
                <input
                  type="text"
                  required
                  value={uploadTitle}
              ...
              <Select
                ...
                options={['Report Templates', 'Presentation Decks', 'Code Skeletons']}
              />
              <div>
