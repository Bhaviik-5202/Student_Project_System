...
export const PreviewModal = ({ resource, isOpen, onClose, onDownload }) => {
  ...
  const isPdf =
    resource.fileType === 'pdf' || resource.url?.toLowerCase().endsWith('.pdf');
          ...
          <div className='flex items-center gap-3 overflow-hidden'>
            ...
            <div className='truncate'>
              ...
              <p className='text-xs text-slate-500 dark:text-slate-400'>
                {resource.category || 'General'} &bull;{' '}
                {resource.fileSize || '1.0 MB'} &bull;{' '}
            ...
            <div className='w-full max-w-md rounded-2xl bg-white dark:bg-slate-900 p-8 text-center shadow-sm border border-slate-200 dark:bg-slate-800 dark:border-slate-700'>
              ...
              <p className='mb-4 text-xs text-slate-500 dark:text-slate-400'>
                {resource.description ||
                  'No additional preview available for this file format.'}
              </p>
        ...
        <div className='p-6 space-y-6 max-h-[75vh] overflow-y-auto'>
          <div>
            ...
            <p className='mt-2 text-sm leading-relaxed text-slate-600 dark:text-slate-300'>
              {resource.description ||
                'No detailed description provided for this academic resource.'}
            </p>
          </div>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 rounded-xl bg-slate-50 dark:bg-slate-800 p-4 dark:bg-slate-700/40 border border-slate-200 dark:border-slate-700 text-xs'>
            <div className='flex items-center gap-2.5 text-slate-700 dark:text-slate-300'>
              ...
              <div>
                ...
                <p className='font-semibold'>
                  {resource.uploadedBy?.name || 'Academic Faculty'}
                </p>
              </div>
            </div>
            <div className='flex items-center gap-2.5 text-slate-700 dark:text-slate-300'>
            ...
            <div className='flex items-center gap-2.5 text-slate-700 dark:text-slate-300'>
              ...
              <div>
                ...
                <p className='font-semibold'>
                  {resource.downloadsCount || 0} times
                </p>
              </div>
            </div>
          </div>
      ...
      <div className='w-full max-w-lg rounded-2xl bg-white dark:bg-slate-900 shadow-2xl dark:bg-slate-800 border border-slate-200 dark:border-slate-700 overflow-hidden'>
        <div className='flex items-center justify-between border-b border-slate-200 p-4 px-6 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80'>
          <h3 className='font-bold text-slate-900 dark:text-white text-base flex items-center gap-2'>
            <Edit size={18} className='text-indigo-600' /> Edit Resource
            Metadata
          </h3>
          <button
            onClick={onClose}
...
export const UploadModal = ({
  ...
  const [category, setCategory] = useState(
    defaultType === 'template' ? 'Academic Templates' : 'Project Documentation'
  );
