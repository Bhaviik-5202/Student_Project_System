...
const getFormatIcon = (fileType, type) => {
  ...
  return {
    icon: FolderOpen,
    color: 'text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 ',
  };
};
...
export const ResourceBrowser = () => {
  ...
  const fetchResources = useCallback(async () => {
    ...
    try {
      ...
      setError(
        'Unable to fetch resource repository. Please check your network connection.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const stats = useMemo(() => {
    ...
    const totalDownloads = resources.reduce(
      (sum, r) => sum + (r.downloadsCount || r.downloads || 0),
      0
    );
  ...
  const handleDelete = async (resource) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setResources((prev) =>
          prev.filter((r) => (r._id || r.id) !== (resource._id || resource.id))
        );
      } else {
