...
async function registerAndLogin() {
  ...
  const token =
    (res.data && (res.data.token || (res.data.data && res.data.data.token))) ||
    res.data.data?.token;
  // fallback: some endpoints return { success, message, data: { token, user }}
  let authToken = token;
  if (!authToken && res.data && res.data.data && res.data.data.token)
    authToken = res.data.data.token;
  if (!authToken && res.data && res.data.token) authToken = res.data.token;

  if (!authToken && res.headers && res.headers.authorization)
...
async function testCrud(path, sampleCreate = {}) {
  ...
  try {
    ...
    if (r.data && (r.data.data || r.data._id))
      result.createdId =
        (r.data.data && r.data.data._id) ||
        r.data._id ||
        (r.data.data && r.data.data.id);
  } catch (e) {
  ...
  if (result.createdId) {
    ...
    // UPDATE
    try {
      const r = await client.put(`${path}/${result.createdId}`, {
        updatedBySmoke: true,
      });
...
async function authTests() {
  ...
  if (r2.status === 200 || r2.status === 201) {
    const token =
      (r2.data && (r2.data.token || (r2.data.data && r2.data.data.token))) ||
      r2.data.data?.token;
...
async function run() {
  ...
  const endpoints = [
    {
      path: '/projects',
      sample: { title: 'Smoke Project', description: 'Created by smoke test' },
    },
    {
      path: '/students',
      sample: {
        firstName: 'Smoke',
        lastName: 'Student',
        email: `smoke.stu+${Date.now()}@example.com`,
      },
    },
    {
      path: '/staff',
      sample: {
        firstName: 'Smoke',
        lastName: 'Staff',
        email: `smoke.staff+${Date.now()}@example.com`,
      },
    },
    { path: '/submissions', sample: { title: 'Smoke Submission' } },
    {
      path: '/meetings',
      sample: { title: 'Smoke Meeting', start: new Date().toISOString() },
    },
    {
      path: '/portfolios',
      sample: { name: 'Smoke Portfolio', description: 'test' },
    },
  ...
  try {
    ...
    if (
      a &&
      a.status === 200 &&
      Array.isArray(a.data && a.data.data ? a.data.data : a.data)
    ) {
  ...
  // write exit code
  const hasCritical = results.endpoints.some(
    (e) =>
      e &&
      ((e.list && e.list.status >= 500) || (e.create && e.create.status >= 500))
  );
