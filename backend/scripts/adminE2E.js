...
(async () => {
  try {
    ...
    const loginRes = await axios.post(`${base}/auth/login`, { email: adminEmail, password: adminPass });
    ...
    const studentRes = await axios.post(`${base}/students`, { name: 'E2E Student', email: stuEmail, rollNumber: roll, department: 'CS', year: 2 }, { headers });
    console.log('STUDENT_CREATE', studentRes.data.message || studentRes.data);
    const studentId = studentRes.data.data._id;

    // Update student
    const studentUpdate = await axios.put(`${base}/students/${studentId}`, { name: 'E2E Student Updated' }, { headers });
    console.log('STUDENT_UPDATE', studentUpdate.data.message || studentUpdate.data);

    // Create staff
    const staffEmail = `staff.e2e+${Date.now()}@example.com`;
    const staffRes = await axios.post(`${base}/staff`, { name: 'E2E Staff', email: staffEmail, role: 'faculty' }, { headers });
    console.log('STAFF_CREATE', staffRes.data.message || staffRes.data);
    const staffId = staffRes.data.data._id;

    // Update staff
    const staffUpdate = await axios.put(`${base}/staff/${staffId}`, { name: 'E2E Staff Updated' }, { headers });
    ...
    let ptype = (typesRes.data && typesRes.data.data || []).find(t => t.name === 'E2E Type');
    if (!ptype) {
      const ptypeRes = await axios.post(`${base}/projects/types`, { name: 'E2E Type', description: 'E2E project type', duration: '6 weeks', maxStudents: 4, category: 'Software' }, { headers });
      ptype = ptypeRes.data.data;
      console.log('PROJECT_TYPE_CREATE', ptypeRes.data.message || ptypeRes.data);
    } else {
    ...
    // Create project
    const projRes = await axios.post(`${base}/projects`, { title: 'E2E Project', description: 'desc', type: ptypeId }, { headers });
    console.log('PROJECT_CREATE', projRes.data.message || projRes.data);
    const projId = projRes.data.data._id;

    // Update project (non-fatal)
    try {
      const projU = await axios.put(`${base}/projects/${projId}`, { description: 'updated' }, { headers });
      console.log('PROJECT_UPDATE', projU.data.message || projU.data);
    } catch (e) {
      console.warn('PROJECT_UPDATE_FAILED', e.response ? (e.response.data || e.response.status) : e.message);
    }

    // Create course (assign to staff) required for assignments
    const courseCode = 'C' + Date.now().toString().slice(-6);
    const courseRes = await axios.post(`${base}/courses`, { name: 'E2E Course', code: courseCode, faculty: staffId }, { headers });
    ...
    const asgRes = await axios.post(`${base}/assignments`, { title: 'E2E Assignment', description: 'desc', dueDate: due, course: courseId }, { headers });
    console.log('ASSIGNMENT_CREATE', asgRes.data.message || asgRes.data);
    const asgId = asgRes.data.data._id;

    // Update assignment
    const asgU = await axios.put(`${base}/assignments/${asgId}`, { title: 'E2E Assignment Updated' }, { headers });
    ...
    console.log('USERS_COUNT', Array.isArray(users.data.data) ? users.data.data.length : 'unknown');
    ...
    console.error('Admin E2E error:', err.response ? (err.response.data || err.response.status) : err.message);
    process.exit(2);
  }
})();
