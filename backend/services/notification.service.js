...
exports.getByUserId = async (userId, page = 1, limit = 10) => {
  try {
    ...
    return response(false, { notifications, total, page, pages: Math.ceil(total / limit) }, "Notifications fetched successfully");
  } catch (err) {
...
exports.getUnreadByUserId = async (userId) => {
  try {
    const notifications = await notificationRepository.findAll({ user: userId, read: false }, { sort: { createdAt: -1 } });
    return response(false, notifications, "Unread notifications fetched successfully");
  } catch (err) {
    return response(true, null, err.message || "Failed to fetch unread notifications");
  }
};
...
exports.markAsRead = async (id, userId) => {
  try {
    ...
    return response(false, notification, "Notification marked as read successfully");
  } catch (err) {
    return response(true, null, err.message || "Failed to mark notification as read");
  }
};
...
exports.markAllAsRead = async (userId) => {
  try {
    const unread = await notificationRepository.findAll({ user: userId, read: false });
    await Promise.all(unread.map(n => notificationRepository.update(n._id, { read: true })));
    return response(false, null, "All notifications marked as read successfully");
  } catch (err) {
    return response(true, null, err.message || "Failed to mark all notifications as read");
  }
};
